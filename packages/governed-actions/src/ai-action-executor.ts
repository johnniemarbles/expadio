...
export function parseGovernedAiActionConfiguration(
  ...
  if (!operation || typeof operation !== "string") {
    throw new Error("AI_ACTION_OPERATION_REQUIRED");
  }
  ...
  const promptKey = (config.promptKey as string) ?? "prompt.default";
  ...
  return {
    ...
    promptKey,
    promptVersion,
    requiredResidencyTags: (config.requiredResidencyTags as string[]) ?? [],
    requiredComplianceTags: (config.requiredComplianceTags as string[]) ?? [],
    ...(config.maximumCostMinorUnits !== undefined ? { maximumCostMinorUnits: Number(config.maximumCostMinorUnits) } : {}),
    ...(config.autoApproveConfidenceThreshold !== undefined ? { autoApproveConfidenceThreshold: Number(config.autoApproveConfidenceThreshold) } : {}),
  };
}
...
export async function executeGovernedAiAction(input: {
  ...
  try {
    ...
    const completedAt = now();
    const threshold = aiConfig.autoApproveConfidenceThreshold ?? 0.9;
    const confidence = proposal.confidence ?? 0;
    const isApproved = confidence >= threshold;
    ...
    const attempt: GovernedActionExecutionAttempt = {
      ...
      startedAt,
      completedAt,
      reasonCode: isApproved ? "AI_PROPOSAL_AUTO_APPROVED" : "AI_PROPOSAL_REQUIRES_REVIEW",
      reason: isApproved
        ? `Confidence ${confidence} meets threshold ${threshold}`
        : `Confidence ${confidence} is below review threshold ${threshold}`,
