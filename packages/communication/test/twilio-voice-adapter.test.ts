...
import { TwilioVoiceAdapter } from '../src/twilio-voice-adapter.ts';
...
test('sends voice call successfully', async () => {
  ...
  assert.equal(String(calls[0]?.input), 'https://api.twilio.com/2010-04-01/Accounts/AC123/Calls.json');
  assert.deepEqual(calls[0]?.init?.headers, {
    Authorization: 'Basic QUMxMjM6YXV0aF90b2tlbg==',
    'Content-Type': 'application/x-www-form-urlencoded',
  });
  assert.equal(String(calls[0]?.init?.body), 'To=%2B1234567890&From=%2B0987654321&Url=https%3A%2F%2Fexample.com%2Ftwiml');
});

test('handles rate limits and auth failures for voice', async () => {
