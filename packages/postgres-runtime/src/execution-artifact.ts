...
export type ExecutionArtifactSourceKind = 'AI_INVOCATION' | 'VOICE_REQUEST';
...
function validate(input: {
  ...
  for (const [name, value] of [
    ...
    if (value.trim() === '') throw new Error(`EXECUTION_ARTIFACT_${name.toUpperCase()}_REQUIRED`);
  }
  ...
  if (input.modelKey !== undefined && input.modelKey !== null && input.modelKey.trim() === '') {
...
export async function persistExecutionArtifact(
...
): Promise<{ readonly artifact: PersistedExecutionArtifact; readonly replayed: boolean }> {
  validate(input);
  ...
  const inserted = await client.query<ExecutionArtifactRow>(
    `INSERT INTO platform.execution_artifacts (
     ...
     ON CONFLICT (tenant_id, artifact_kind, source_kind, source_id) DO NOTHING
     RETURNING ${COLUMNS}`,
    [
      input.tenantId,
      input.artifactKind,
  ...
  if (created !== undefined) return { artifact: mapRow(created), replayed: false };
  ...
  if (row === undefined) throw new Error('EXECUTION_ARTIFACT_IDEMPOTENCY_CONFLICT');
