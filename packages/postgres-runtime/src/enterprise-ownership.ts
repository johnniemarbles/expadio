...
export async function requestEnterpriseOwnershipChange(
  ...
  if (replay) {
    ...
    const exact =
      ...
      && payload.ownerLegalEntityId === ownerLegalEntityId
      && payload.subjectLegalEntityId === subjectLegalEntityId
      && payload.interestType === interestType
      && (payload.percentage ?? null) === percentage;
