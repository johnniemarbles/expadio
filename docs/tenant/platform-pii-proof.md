# Platform PII proof — Wave 1 source closed

Keep #499 draft. Wave 1 source/CI for the dual-shell product contract is closed on this tip. This is still not authenticated e2e and not a merge.
...
- Brand Communications surface observes COMMUNICATE + DELIVERY from that same read. Still no mutations.
...
- Source-scan CI on this branch is green (`check`, `integration`, `postgres-contract`, architecture).

## Not closed — needs a live Railway preview
...
- Separate Brand Next host on `app.expadio.com`. `/brand` is the same-origin fallback only.
