 ...
 * credential-lease boundary. Returns null when no connector is routed so the
 * caller can fall back to manual DNS instructions. A present connector must
 * have a credential reference that resolves through the delegated secret
 * resolver; plaintext never comes from request input or environment variables.
 */
export async function resolveGovernedCloudflareDnsToken(
  ...
  assertLeaseIsCurrent(lease, options.requestedAt);
