...
test('Brand journey fact SQL never selects configuration or recipients', () => {
  ...
  assert.match(FROZEN_EXECUTOR_FACT_QUERY, /communication_deliveries/);
  assert.match(FROZEN_EXECUTOR_FACT_QUERY, /idempotency_key/);
  ...
  assert.doesNotMatch(FROZEN_EXECUTOR_FACT_QUERY, /dispatch_snapshot/);
  assert.doesNotMatch(FROZEN_EXECUTOR_FACT_QUERY, /last_reason/);
...
test('Brand journey runtime folds facts after membership; Platform correlation does not query executors', () => {
  ...
  assert.doesNotMatch(platformCorrelation, /communication_deliveries/);
  assert.doesNotMatch(platformCorrelation, /readFrozenExecutorRows/);
});

test('succeeded schedule stays queued; provider DELIVERED marks communicate', () => {
  const scheduled = journeyFactsFromRows('CS-104', [
    { correlation: 'CS-104', executor: 'SCHEDULE', attemptStatus: 'SUCCEEDED' },
  ]);
  assert.equal(scheduled[0]?.state, 'queued');
  const delivered = journeyFactsFromRows('CS-104', [
    {
      correlation: 'CS-104',
