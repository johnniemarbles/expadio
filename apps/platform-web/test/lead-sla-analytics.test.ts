...
const sla = read('../../../infra/db/migrations/0147_lead_task_sla.sql');
const analytics = read('../../../infra/db/migrations/0148_lead_analytics.sql');
...
test('analytics views use organization_closure for hierarchy-safe rollups', () => {
  ...
  assert.match(analytics, /ancestor_organization_id AS organization_id/);
  assert.match(analytics, /descendant_organization_id = .*organization_id/);
