...
const prospectRoute = read('../app/api/gtm/prospects/route.ts');
...
test('verticals and factory routes bind the four GTM work types', () => {
  ...
  assert.match(replyRoute, /classifyReplyBody/);
  assert.match(prospectRoute, /scoreProspectObservation/);
  assert.match(prospectRoute, /INSERT INTO platform\.gtm_prospect_observations/);
  assert.match(enginesSeam, /scoringMayAutoApprove/);
...
test('console, nav, lead source and review queue know AutoGTM', () => {
  ...
  assert.match(client, /Observe prospect/);
