...
test('theme persistence separates complete profiles from bounded Brand patches',()=>{
  ...
  const governance=read('../../../packages/ui/src/governed-theme.ts');
  ...
  assert.match(governance,/isThemeOverride/);
  assert.match(governance,/THEME_GEOMETRY_OVERRIDE_LOCKED/);
  assert.match(governance,/governedThemeProfileValidator/);
  assert.match(governance,/resolveThemeOverride\(theme, candidate\.value\)/);
});
