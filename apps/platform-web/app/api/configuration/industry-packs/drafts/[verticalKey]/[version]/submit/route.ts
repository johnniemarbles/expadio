...
export async function POST(
  ...
  try {
    ...
    const outcome = await withTenantTransaction(context, async (client) => {
      ...
      try {
        const submitted = await repository.transitionLifecycle({
          scope,
          identity: current.identity,
          expectedState: 'DRAFT',
          next,
        });
        return { kind: 'SUBMITTED' as const, version: submitted };
      } catch (error) {
        if (
      ...
      return NextResponse.json(
        {
          denied: true,
          reasonKey: 'FORBIDDEN',
          message: 'You need a governing role to submit Industry Packs for review.',
        },
        { status: 403 },
      );
    }
    ...
    if (outcome.kind === 'STATE_CONFLICT') {
      return NextResponse.json(
        {
          error: 'Only DRAFT Industry Pack versions can be submitted for review.',
    ...
    if (outcome.kind === 'DEFINITION_INVALID') {
      return NextResponse.json(
        {
          error: 'The persisted Industry Pack definition is invalid and cannot enter review.',
