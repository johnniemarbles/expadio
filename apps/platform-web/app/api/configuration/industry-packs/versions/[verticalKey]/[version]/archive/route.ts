  ...
  try {
    ...
    const outcome = await withTenantTransaction(context, async (client) => {
      ...
      try {
        ...
        return { kind: 'ARCHIVED' as const, version: archived };
      } catch (error) {
        if (
      ...
      return NextResponse.json(
        {
          denied: true,
          reasonKey: 'FORBIDDEN',
          message: 'You need a governing role to archive Industry Packs.',
        },
        { status: 403 },
      );
    }
    if (outcome.kind === 'NOT_FOUND') {
      return NextResponse.json({ error: 'Industry Pack version was not found.' }, { status: 404 });
    }
    if (outcome.kind === 'STATE_CONFLICT') {
      return NextResponse.json(
        {
          error: 'That Industry Pack version cannot be archived from its current state.',
