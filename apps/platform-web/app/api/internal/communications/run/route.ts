...
import {
  ...
  type CommunicationDeliveryWorkerResult,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const serviceSubjectId =
      process.env.EXPADIO_COMMUNICATION_WORKER_SUBJECT_ID?.trim() ?? '';
    if (serviceSubjectId === '') {
      throw new InternalWorkerAuthError(
        503,
        'COMMUNICATION_WORKER_IDENTITY_DISABLED',
        'Communication worker service identity is not configured.',
      );
    }
    ...
    const results: CommunicationDeliveryWorkerResult[] = [];
    ...
    return NextResponse.json(
      {
        ok: false,
        error: 'Communication delivery worker execution failed.',
        reasonCode: 'COMMUNICATION_WORKER_EXECUTION_FAILED',
      },
      { status: 500 },
    );
  } finally {
