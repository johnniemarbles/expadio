...
export async function POST(
  ...
  try {
    ...
    if (idempotencyKey.length < 8 || idempotencyKey.length > 256 || /[\r\n\t]/u.test(idempotencyKey)) {
      return NextResponse.json({ error: 'A stable idempotencyKey (8–256 characters) is required.' }, { status: 400 });
    }
    ...
    const result = await withTenantTransaction(context, async (client) => {
      ...
      const connectors = await providerRegistry.listConnectors(context.tenantId, CAPABILITY_KEY);
      const policy = await providerRegistry.loadRoutingPolicy(context.tenantId, CAPABILITY_KEY);

      const selected = connectors.filter((connector) => connector.connectorKey === connectorKey);
      ...
      const routed = routePreparedCommunicationDispatch(dispatch, selected, policy ?? undefined);
      if (!routed.routed) {
        return {
          status: 409 as const,
          body: {
      ...
      if (routed.connector.providerKey !== 'resend' || routed.connector.providerType !== 'email') {
        return {
          status: 400 as const,
          body: { error: 'This test-send boundary currently supports Resend email connectors only.' },
        };
      }
      ...
      if (!senderPrepared.ok) {
        return {
          status: 409 as const,
          body: { error: 'Create and verify a tenant sender identity before testing.', reasonCode: senderPrepared.reasonCode },
        };
      }

      const credentialReference = await new PostgresConnectorCredentialRepository(client)
        .loadCredentialReference(context.tenantId, connectorKey);
      if (credentialReference === null) {
        return {
          status: 409 as const,
          body: { error: 'The selected connector has no active credential reference.' },
        };
      }

      const resolved = await delegatedSecretResolver.resolve(credentialReference);
