  ...
  try {
    ...
    const facts = await withTenantClient(context, async (client) => {
      ...
      const domains = await client.query(
        `SELECT
           count(DISTINCT lower(split_part(address, '@', 2))) FILTER (
             WHERE verification_status = 'VERIFIED' AND status = 'ACTIVE'
           )::int AS verified,
           count(DISTINCT lower(split_part(address, '@', 2)))::int AS total
    ...
    const dashboardState = facts.withCredential === 0 ? 'UNCONFIGURED' : degradedReasons.length > 0 ? 'DEGRADED' : 'STEADY';
