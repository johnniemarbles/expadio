...
/**
 * Brand template cloning (PR #225/#226).
 *
 * Clones an active PLATFORM template into a tenant-owned DRAFT, preserving the
 * exact source template/version lineage. Tenant-scoped: authored under the
 * caller's real tenant (not the demo scaffold), gated on a TENANT_OWNER /
 * TENANT_ADMIN role, and RLS-bound via app.tenant_id. A per-(tenant, trigger,
 * channel, locale) advisory lock serialises concurrent clones so two requests
  ...
  try {
    ...
    try {
      ...
      const role = await client.query(
        ...
        [context.tenantId, context.subjectId, BRAND_TEMPLATE_ROLES],
      );
      ...
      // Serialise clones for the same tenant/template key so two concurrent
      // requests cannot both pass the duplicate check and create duplicate drafts.
      await client.query(
    ...
    if (err?.denied) return NextResponse.json(err.denied, { status: err.status ?? 403 });
