...
/**
 * Record an immutable decision against a vendor's current workflow stage — the
 * same governed decision capture as a CRM case, on a non-CRM subject. This is
 * what lets the decision-required APPROVAL stage advance to ACTIVE: the exit
 * gate blocks the transition until a decision with an allowed outcome exists.
 *
 * vendor.onboarding registers no authority deriver, so the decision is gated by
 * role and separation of duties alone (approver must hold a governing role and
 * differ from the subject who moved the vendor into APPROVAL).
 */
