 ...
 * CRM cases, on a non-CRM subject.
 *
 *  GET   — the vendor's workflow instance (if started) and the blueprint's stages.
 *  POST  — start the vendor.onboarding workflow for the vendor.
 *  PATCH — advance the vendor's instance to a target stage (append-only history).
 *
 * Reads require membership; writes require a governing role. The vendor's
 * stage_key mirrors the instance's current stage, and its status flips to ACTIVE
 * once onboarding reaches the final stage.
 */

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

const SUBJECT_TYPE = 'vendor';

/** Business status derived from the workflow stage the vendor sits at. */
function vendorStatusForStage(stageKey: string | null): string {
  return stageKey === 'ACTIVE' ? 'ACTIVE' : 'PENDING';
}
  ...
  try {
    ...
    const result = await withTenantClient(context, async (client) => {
      const row = await client.query(
        `SELECT workflow_instance_id, blueprint_key, stage_key FROM platform.vendors WHERE vendor_id = $1::uuid`,
        [vendorId],
      );
  ...
  try {
    ...
    const result = await withTenantClient(context, async (client) => {
      ...
      try {
        ...
        const row = await client.query(
          `SELECT blueprint_key, workflow_instance_id FROM platform.vendors WHERE vendor_id = $1::uuid FOR UPDATE`,
          [vendorId],
        );
        ...
        await client.query(
          ...
          [vendorId, started.instance.instanceId, started.instance.currentStageKey ?? null, vendorStatusForStage(started.instance.currentStageKey ?? null)],
        );
    ...
    if ('already' in result) {
      return NextResponse.json({ error: 'This vendor already has a workflow.' }, { status: 409 });
    }
    if ('noBlueprintKey' in result || 'noBlueprint' in result) {
      return NextResponse.json({ error: 'No active vendor.onboarding blueprint is available.' }, { status: 409 });
    }
  ...
  try {
    ...
    const result = await withTenantClient(context, async (client) => {
      ...
      try {
        await context.applyTo(client);
        const row = await client.query(
          `SELECT workflow_instance_id FROM platform.vendors WHERE vendor_id = $1::uuid FOR UPDATE`,
          [vendorId],
        );
        if (row.rows.length === 0) {
          await client.query('ROLLBACK');
        ...
        await client.query(
          `UPDATE platform.vendors SET stage_key = $2, status = $3, updated_at = now() WHERE vendor_id = $1::uuid`,
          [vendorId, moved.instance.currentStageKey ?? null, vendorStatusForStage(moved.instance.currentStageKey ?? null)],
        );
        await client.query('COMMIT');
