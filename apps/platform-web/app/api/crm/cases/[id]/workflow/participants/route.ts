...
/**
 * Assign a participant to a stage's semantic slot (e.g. "reviewer" on REVIEW).
 * Entering a stage is gated until its required slots are filled. Governed by a
 * tenant role; the assignment is tenant-scoped by RLS. When no explicit target
 * is given, the caller is assigned as a USER — the minimal real action.
 */
