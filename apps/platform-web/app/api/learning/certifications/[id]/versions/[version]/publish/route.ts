...
export async function POST(
  ...
  try {
    ...
    const certificationId = requireLearningUuid(decodeURIComponent(raw.id), 'certificationId');
    ...
    const result = await withTenantTransaction(context, async (client) => {
      ...
      return {
        published: await publishLearningCertificationVersion(client, {
    ...
    if ('forbidden' in result) {
      return NextResponse.json(
        { denied: true, reasonKey: 'FORBIDDEN', message: 'Learning certification publication requires a tenant administrator role.' },
        { status: 403 },
      );
    }
