...
export async function POST(
  ...
  try {
    ...
    const programId = requireLearningUuid(decodeURIComponent(raw.id), 'programId');
    ...
    const result = await withTenantTransaction(context, async (client) => {
      ...
      return {
        published: await publishLearningProgramVersion(client, {
    ...
    if ('forbidden' in result) {
      return NextResponse.json(
        { denied: true, reasonKey: 'FORBIDDEN', message: 'Learning program publication requires a tenant administrator role.' },
        { status: 403 },
      );
    }
