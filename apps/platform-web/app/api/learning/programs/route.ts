...
export async function GET(request: Request) {
  try {
    ...
    const result = await withTenantTransaction(context, async (client) => {
      ...
      return { programs: await listLearningPrograms(client, context.tenantId) } as const;
    });
    if ('forbidden' in result) {
      return NextResponse.json(
        { denied: true, reasonKey: 'FORBIDDEN', message: 'Learning program administration requires a tenant administrator role.' },
...
export async function POST(request: Request) {
  try {
    ...
    const result = await withTenantTransaction(context, async (client) => {
      if (!(await hasLearningAuthoringRole(client, context.subjectId))) return { forbidden: true } as const;
      return {
        program: await createLearningProgram(client, {
    ...
    return NextResponse.json(result.program, { status: 201, headers: { 'Cache-Control': 'private, no-store' } });
  } catch (error) {
    const mapped = learningApiError(error);
    if (mapped !== null) return NextResponse.json(mapped.body, { status: mapped.status, headers: { 'Cache-Control': 'private, no-store' } });
    const { body, status } = deniedResponse(error);
