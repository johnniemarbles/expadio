...
export async function POST(
  ...
  try {
    ...
    const questionId = requireLearningUuid(decodeURIComponent(raw.id), 'questionId');
    ...
    const result = await withTenantTransaction(context, async (client) => {
      ...
      return {
        published: await publishLearningQuestionVersion(client, {
    ...
    if ('forbidden' in result) {
      return NextResponse.json(
        { denied: true, reasonKey: 'FORBIDDEN', message: 'Learning assessment publication requires a tenant administrator role.' },
        { status: 403 },
      );
    }
