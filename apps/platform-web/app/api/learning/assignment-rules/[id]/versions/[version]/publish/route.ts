...
import { publishLearningAssignmentRuleVersion } from '@expadio/postgres-runtime/learning-assignment-automation';
import {
  deniedResponse,
...
export async function POST(
  ...
  try {
    ...
    const assignmentRuleId = requireLearningUuid(
      decodeURIComponent(raw.id),
      'assignmentRuleId',
    );
    ...
    const result = await withTenantTransaction(context, async (client) => {
      ...
      return {
        published: await publishLearningAssignmentRuleVersion(client, {
      ...
      return NextResponse.json(
        {
          denied: true,
          reasonKey: 'FORBIDDEN',
          message: 'Learning assignment rule publication requires a tenant administrator role.',
        },
        { status: 403 },
      );
    }
