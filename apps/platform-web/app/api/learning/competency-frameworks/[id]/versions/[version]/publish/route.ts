...
import { publishLearningCompetencyFrameworkVersion } from '@expadio/postgres-runtime/learning-competency';
import {
  deniedResponse,
...
export async function POST(
  ...
  try {
    ...
    const competencyFrameworkId = requireLearningUuid(
    ...
    const result = await withTenantTransaction(context, async (client) => {
      ...
      return {
        published: await publishLearningCompetencyFrameworkVersion(client, {
      ...
      return NextResponse.json(
        {
          denied: true,
          reasonKey: 'FORBIDDEN',
          message: 'Learning competency publication requires a tenant administrator role.',
        },
        { status: 403 },
      );
    }
