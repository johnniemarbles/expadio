...
import { listLearningCredentials } from '@expadio/postgres-runtime/learning-program-certification';
import {
  deniedResponse,
...
export async function GET(request: Request) {
  try {
    ...
    const result = await withTenantTransaction(context, async (client) => {
      ...
      return {
        credentials: await listLearningCredentials(client, {
    ...
    if ('forbidden' in result) {
      return NextResponse.json(
        { denied: true, reasonKey: 'FORBIDDEN', message: 'Learning credential administration requires a tenant administrator role.' },
