...
import { approveEnterpriseJurisdictionActivation } from '@expadio/postgres-runtime/enterprise-commercial';
...
export async function POST(
  ...
  try {
    ...
    const value = await withTenantTransaction(context, async (client) => {
      ...
      return approveEnterpriseJurisdictionActivation(client, {
    ...
    if ('forbidden' in value) {
      return NextResponse.json(
        { denied: true, reasonKey: 'ENTERPRISE_JURISDICTION_APPROVAL_FORBIDDEN', message: 'This jurisdiction is outside the selected governing scope.' },
        { status: 403 },
      );
    }
