...
const badge = (state: string): React.CSSProperties => {
  const map: Record<string, string> = {
    ACTIVE: '#0f766e', DRAFT: '#64748b', IN_REVIEW: '#b45309', SUPERSEDED: '#94a3b8', ARCHIVED: '#94a3b8',
  };
  return {
    display: 'inline-block', padding: '2px 8px', borderRadius: 999, fontSize: 11, fontWeight: 600,
    color: '#fff', background: map[state] ?? '#64748b',
  };
};
                ...
                <td>
                  {b.scope === 'TENANT' && (b.state === 'DRAFT' || b.state === 'IN_REVIEW') ? (
                    <button
                      ...
                      disabled={busy !== null}
                      style={{ padding: '4px 10px', borderRadius: 6, border: 'none', background: '#0f766e', color: '#fff', fontSize: 12, fontWeight: 600, cursor: busy ? 'default' : 'pointer' }}
                    >
