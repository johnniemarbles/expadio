...
export function GtmClient() {
  ...
  async function file(kind: 'icp' | 'sequence' | 'campaign') {
    ...
    try {
      ...
      await readJson<unknown>(res, 'Could not file draft.');
      setName('');
      await refresh();
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'Could not file draft.');
    } finally {
      setBusy(null);
    }
  }
  ...
  async function startReview(kind: WorkflowKind, id: string) {
    ...
    try {
      ...
      await readJson<unknown>(res, 'Could not start review.');
      await refresh();
    } catch (cause) {
  ...
  async function assignReviewer(kind: WorkflowKind, id: string, stageKey: string, participantKey: string) {
    ...
    try {
      ...
      await readJson<unknown>(res, 'Could not assign reviewer.');
      await refresh();
    } catch (cause) {
  ...
  async function decide(kind: WorkflowKind, id: string, outcome: 'APPROVE' | 'REJECT') {
    ...
    try {
      ...
      await readJson<unknown>(res, 'Could not record the decision.');
      await refresh();
    } catch (cause) {
  ...
  async function fileIntent(sequenceId: string) {
    ...
    try {
      const res = await fetch(`/api/gtm/sequences/${encodeURIComponent(sequenceId)}/communicate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ stepKey: 'touch-1', subject: touchSubject, body: touchBody, recipientEmail: prospectEmail }),
      });
      const data = await readJson<Record<string, unknown>>(res, 'Could not file Communication intent.');
  ...
  async function fileMeeting() {
    ...
    try {
      ...
      await readJson<unknown>(res, 'Could not file meeting request.');
      setMeetingSummary('');
      await refresh();
    } catch (cause) {
  ...
  async function observeProspect() {
    ...
    try {
      const res = await fetch('/api/gtm/prospects', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brandId, email: prospectEmail, title: prospectTitle, icpId: icps[0]?.icp_id }),
      });
      await readJson<unknown>(res, 'Could not observe prospect.');
      await refresh();
    } catch (cause) {
  ...
  async function ingestReply() {
    ...
    try {
      const res = await fetch('/api/gtm/replies', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brandId, fromEmail: prospectEmail, rawPayload: { body: replyBody } }),
      });
      await readJson<unknown>(res, 'Could not ingest reply.');
      setReplyBody('');
      await refresh();
    } catch (cause) {
  ...
  return (
    <OperationalStateBoundary state={dashboardState} onRetry={() => void refresh(true)}>
      <section className={styles.root} aria-labelledby="gtm-title">
        <header className={styles.header}>
          <div>
            ...
            <p className={styles.description}>
              Govern ICPs, sequences, campaigns, meeting requests, prospect observations, and reply ingestion through the workflow and communication-intent path. No message is sent from this surface.
            </p>
          </div>
        ...
        {!isConfigured ? (
          <section className={styles.callout} aria-labelledby="gtm-setup-title">
            <h2 id="gtm-setup-title">Configure a Brand context to start</h2>
            <p>Enter the Brand UUID used by the GTM APIs, then file an ICP, sequence, or campaign draft. Metrics remain suppressed until live records exist.</p>
          </section>
        ) : (
        ...
        <section className={styles.panel} aria-labelledby="gtm-drafts-title">
          <div className={styles.panelHead}><div><p className={styles.eyebrow}>Drafting</p><h2 id="gtm-drafts-title">File governed GTM drafts</h2></div></div>
          <div className={styles.panelBody}>
            <div className={styles.formGrid}>
              <div className={styles.field}><label htmlFor="gtm-name">Draft name</label><input id="gtm-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Enter a governed draft name" /></div>
              <div className={styles.field}><label htmlFor="gtm-brand">Brand UUID</label><input id="gtm-brand" value={brandId} onChange={(event) => setBrandId(event.target.value)} placeholder="Brand context UUID" /></div>
            </div>
            <div className={styles.actionRow}>
              <button type="button" className={styles.button} disabled={busy !== null || !name.trim() || !brandId.trim()} onClick={() => void file('icp')}>Propose ICP</button>
              <button type="button" className={styles.secondaryButton} disabled={busy !== null || !name.trim() || !brandId.trim()} onClick={() => void file('sequence')}>File sequence draft</button>
              <button type="button" className={styles.secondaryButton} disabled={busy !== null || !name.trim() || !brandId.trim()} onClick={() => void file('campaign')}>File campaign draft</button>
            </div>
          </div>
        </section>

        <section className={styles.panel} aria-labelledby="gtm-prospect-title">
          <div className={styles.panelHead}><div><p className={styles.eyebrow}>Signals</p><h2 id="gtm-prospect-title">Prospects, meetings, and replies</h2></div></div>
          <div className={styles.panelBody}>
            <div className={styles.formGrid}>
              <div className={styles.field}><label htmlFor="gtm-email">Prospect email</label><input id="gtm-email" value={prospectEmail} onChange={(event) => setProspectEmail(event.target.value)} placeholder="Recipient or sender email" /></div>
              <div className={styles.field}><label htmlFor="gtm-title-field">Prospect title</label><input id="gtm-title-field" value={prospectTitle} onChange={(event) => setProspectTitle(event.target.value)} placeholder="Known role title" /></div>
              <div className={styles.field}><label htmlFor="gtm-meeting">Meeting summary</label><input id="gtm-meeting" value={meetingSummary} onChange={(event) => setMeetingSummary(event.target.value)} placeholder="Summarize the request" /></div>
              <div className={styles.field}><label htmlFor="gtm-reply">Reply body</label><textarea id="gtm-reply" value={replyBody} onChange={(event) => setReplyBody(event.target.value)} placeholder="Paste received reply text" /></div>
            </div>
            <div className={styles.actionRow}>
              <button type="button" className={styles.button} disabled={busy !== null || !brandId.trim() || !prospectEmail.trim()} onClick={() => void observeProspect()}>Observe prospect</button>
              <button type="button" className={styles.secondaryButton} disabled={busy !== null || !brandId.trim() || !prospectEmail.trim() || !meetingSummary.trim()} onClick={() => void fileMeeting()}>File meeting request</button>
              <button type="button" className={styles.secondaryButton} disabled={busy !== null || !brandId.trim() || !prospectEmail.trim() || !replyBody.trim()} onClick={() => void ingestReply()}>Ingest reply</button>
            </div>
          </div>
        </section>

        <section className={styles.panel} aria-labelledby="gtm-touch-title">
          <div className={styles.panelHead}><div><p className={styles.eyebrow}>Communication intent</p><h2 id="gtm-touch-title">Sequence touch payload</h2></div></div>
          <div className={styles.panelBody}>
            <div className={styles.formGrid}>
              <div className={styles.field}><label htmlFor="gtm-subject">Touch subject</label><input id="gtm-subject" value={touchSubject} onChange={(event) => setTouchSubject(event.target.value)} placeholder="Subject from approved sequence" /></div>
              <div className={styles.field}><label htmlFor="gtm-body">Touch body</label><textarea id="gtm-body" value={touchBody} onChange={(event) => setTouchBody(event.target.value)} placeholder="Body from approved sequence" /></div>
            </div>
          </div>
        </section>

        <section className={styles.panel} aria-labelledby="gtm-icps-title">
          <div className={styles.panelHead}><div><p className={styles.eyebrow}>Review fabric</p><h2 id="gtm-icps-title">ICP proposals</h2></div><span className={styles.pill}>{icps.length} records</span></div>
          <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Name</th><th>Status</th><th>Stage</th><th>Actions</th></tr></thead><tbody>{icps.map((row) => <tr key={row.icp_id}><td><strong>{row.name}</strong></td><td><StatusPill value={`${row.status}/${row.review_status}`} /></td><td>{row.stage_key ?? '—'}</td><td><div className={styles.actionRow}>{row.workflow_instance_id === null ? <button type="button" className={styles.secondaryButton} disabled={busy !== null} onClick={() => void startReview('icps', row.icp_id)}>Start review</button> : <><button type="button" className={styles.secondaryButton} disabled={busy !== null} onClick={() => void assignReviewer('icps', row.icp_id, 'GOVERNANCE_REVIEW', 'gtm_reviewer')}>Assign reviewer</button><button type="button" className={styles.button} disabled={busy !== null} onClick={() => void decide('icps', row.icp_id, 'APPROVE')}>Approve</button><button type="button" className={styles.secondaryButton} disabled={busy !== null} onClick={() => void decide('icps', row.icp_id, 'REJECT')}>Reject</button></>}</div></td></tr>)}{icps.length === 0 ? <tr><td colSpan={4} className={styles.empty}>No ICP proposals yet.</td></tr> : null}</tbody></table></div>
        </section>

        <section className={styles.panel} aria-labelledby="gtm-sequences-title">
          <div className={styles.panelHead}><div><p className={styles.eyebrow}>Sequences</p><h2 id="gtm-sequences-title">Sequence drafts</h2></div><span className={styles.pill}>{sequences.length} records</span></div>
          <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Name</th><th>Status</th><th>Stage</th><th>Actions</th></tr></thead><tbody>{sequences.map((row) => <tr key={row.sequence_id}><td><strong>{row.name}</strong></td><td><StatusPill value={row.status} /></td><td>{row.stage_key ?? '—'}</td><td><div className={styles.actionRow}>{row.workflow_instance_id === null ? <button type="button" className={styles.secondaryButton} disabled={busy !== null} onClick={() => void startReview('sequences', row.sequence_id)}>Start review</button> : <><button type="button" className={styles.secondaryButton} disabled={busy !== null} onClick={() => void assignReviewer('sequences', row.sequence_id, 'COPY_REVIEW', 'gtm_reviewer')}>Assign reviewer</button><button type="button" className={styles.button} disabled={busy !== null} onClick={() => void decide('sequences', row.sequence_id, 'APPROVE')}>Approve</button><button type="button" className={styles.secondaryButton} disabled={busy !== null || !touchSubject.trim() || !touchBody.trim() || !prospectEmail.trim()} onClick={() => void fileIntent(row.sequence_id)}>File Communication intent</button></>}</div></td></tr>)}{sequences.length === 0 ? <tr><td colSpan={4} className={styles.empty}>No sequence drafts yet.</td></tr> : null}</tbody></table></div>
        </section>

        <section className={styles.panel} aria-labelledby="gtm-campaigns-title">
          <div className={styles.panelHead}><div><p className={styles.eyebrow}>Campaigns</p><h2 id="gtm-campaigns-title">Campaign drafts</h2></div><span className={styles.pill}>{campaigns.length} records</span></div>
          <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Name</th><th>Status</th><th>Stage</th><th>Actions</th></tr></thead><tbody>{campaigns.map((row) => <tr key={row.campaign_id}><td><strong>{row.name}</strong></td><td><StatusPill value={row.status} /></td><td>{row.stage_key ?? '—'}</td><td><div className={styles.actionRow}>{row.workflow_instance_id === null ? <button type="button" className={styles.secondaryButton} disabled={busy !== null} onClick={() => void startReview('campaigns', row.campaign_id)}>Start review</button> : <><button type="button" className={styles.secondaryButton} disabled={busy !== null} onClick={() => void assignReviewer('campaigns', row.campaign_id, 'LAUNCH_REVIEW', 'gtm_reviewer')}>Assign reviewer</button><button type="button" className={styles.button} disabled={busy !== null} onClick={() => void decide('campaigns', row.campaign_id, 'APPROVE')}>Approve</button></>}</div></td></tr>)}{campaigns.length === 0 ? <tr><td colSpan={4} className={styles.empty}>No campaign drafts yet.</td></tr> : null}</tbody></table></div>
        </section>

        <section className={styles.panel} aria-labelledby="gtm-signals-title">
          <div className={styles.panelHead}><div><p className={styles.eyebrow}>Observed demand</p><h2 id="gtm-signals-title">Prospects, meetings, and replies</h2></div></div>
          <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Type</th><th>Subject</th><th>Status</th><th>Detail</th></tr></thead><tbody>{prospects.map((row) => <tr key={row.observation_id}><td>Prospect</td><td><strong>{row.payload?.email ?? '—'}</strong></td><td><StatusPill value={row.status} /></td><td>Score {row.fit_score} · {row.payload?.score?.band ?? '—'} · {row.payload?.title ?? '—'}</td></tr>)}{meetings.map((row) => <tr key={row.meeting_request_id}><td>Meeting</td><td><strong>{row.prospect_email}</strong></td><td><StatusPill value={row.status} /></td><td>{row.summary || '—'} · {row.stage_key ?? '—'}</td></tr>)}{replies.map((row) => <tr key={row.reply_id}><td>Reply</td><td><strong>{row.from_email}</strong></td><td><StatusPill value={row.proposed_class} /></td><td>{row.created_at}</td></tr>)}{prospects.length + meetings.length + replies.length === 0 ? <tr><td colSpan={4} className={styles.empty}>No prospect, meeting, or reply signals yet.</td></tr> : null}</tbody></table></div>
        </section>
      </section>
