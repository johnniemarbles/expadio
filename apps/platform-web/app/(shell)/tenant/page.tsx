...
export default function TenantWorkspacePage() {
  ...
  useEffect(() => { let active = true; setLoading(true); fetch('/api/tenant/work', { cache: 'no-store' }).then((response) => response.ok ? response.json() : null).then((rows) => { if (active && Array.isArray(rows) && rows.length) setWork(rows.map((row: any) => ({ id: row.work_item_id, title: row.title, type: row.work_type, location: row.location_id || 'Permitted location', status: (row.status === 'AWAITING_REVIEW' ? 'Awaiting review' : row.status === 'OUTCOME_UNCERTAIN' ? 'Failed' : row.status.charAt(0) + row.status.slice(1).toLowerCase()) as Status, maker: row.maker_subject_id === 'current-user' ? 'You' : row.maker_subject_id || 'Team' }))); }).catch(() => undefined).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, []);
