        ...
        <div className={styles.stack}>
          ...
          <section className={styles.panel}>
            ...
            <form className={styles.formGrid} onSubmit={requestOwnershipChange}>
              <label>Owner<select name="ownerLegalEntityId" required defaultValue=""><option value="" disabled>Select verified owner</option>{data.legalEntities.map((entity) => <option value={entity.legal_entity_id} key={'owner:' + entity.legal_entity_id}>{entity.legal_name}</option>)}</select></label>
              <label>Owned / controlled entity<select name="subjectLegalEntityId" required defaultValue=""><option value="" disabled>Select verified subject</option>{data.legalEntities.map((entity) => <option value={entity.legal_entity_id} key={'subject:' + entity.legal_entity_id}>{entity.legal_name}</option>)}</select></label>
              <label>Interest type<select name="interestType" defaultValue="EQUITY"><option>EQUITY</option><option>VOTING</option><option>ECONOMIC</option><option>CONTROL</option><option>BENEFICIAL</option></select></label>
              <label>Percentage<input name="percentage" type="number" min="0" max="100" step="0.0001" placeholder="e.g. 60" /></label>
              <label className={styles.wide}>Evidence reference<input name="evidenceRef" placeholder="document:cap-table:2026-09" /></label>
            ...
            <div className={styles.tableWrap}>
              <table>
                ...
                <tbody>{data.ownershipInterests.map((interest) => (
                ...
                <tr key={item.enterprise_change_request_id}>
                  ...
                  <td>
                    {item.operation === 'CHANGE_OWNERSHIP' ? (
                      <div className={styles.inlineAction}>
                        <button disabled={busy !== null} onClick={() => void mutate('ownership:approve:' + item.enterprise_change_request_id, '/api/enterprise/change-requests/' + item.enterprise_change_request_id + '/decision', { action: 'APPROVE', reason: 'Ownership evidence reviewed and approved.' }, 'Ownership change approved and published to the Ownership / Legal graph.')}>Approve</button>
                        <button className={styles.secondaryButton} disabled={busy !== null} onClick={() => void mutate('ownership:reject:' + item.enterprise_change_request_id, '/api/enterprise/change-requests/' + item.enterprise_change_request_id + '/decision', { action: 'REJECT', reason: 'Ownership change rejected during independent review.' }, 'Ownership change rejected.')}>Reject</button>
                      </div>
