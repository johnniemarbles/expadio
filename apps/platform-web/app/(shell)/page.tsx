...
export default async function OverviewPage({ searchParams }: { searchParams: RouteSearchParams }) {
  ...
  return <>
    ...
    <div className={styles.dashboardGrid}>
      ...
      <section className={styles.panel} aria-labelledby="modules-title"><div className={styles.panelHeading}><div><p className={styles.eyebrow}>Tenant products</p><h2 id="modules-title">Apps & modules</h2></div><Link href={"/modules"+context} className={styles.textButton}>Manage →</Link></div><div style={{ padding: "18px 19px", color: "var(--ink-600)", fontSize: 14, lineHeight: 1.5 }}>Review plan entitlement, installation state and activation for shared tenant modules such as Learning.</div></section>
    </div>
