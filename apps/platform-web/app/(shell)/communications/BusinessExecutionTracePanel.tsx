    ...
    <section className={styles.attentionTablePanel}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(160px, 1fr))", gap: 10, marginBottom: 12 }}>
        <label style={{ fontSize: 12, display: "grid", gap: 4 }}>
          Root event id
          <input value={eventId} onChange={(e) => setEventId(e.target.value)} placeholder="uuid" style={{ padding: "8px 10px", borderRadius: "var(--theme-radius-card)", border: "1px solid var(--theme-border)", fontSize: 13 }} />
        </label>
        <label style={{ fontSize: 12, display: "grid", gap: 4 }}>
          Correlation id
          <input value={correlationId} onChange={(e) => setCorrelationId(e.target.value)} placeholder="correlation" style={{ padding: "8px 10px", borderRadius: "var(--theme-radius-card)", border: "1px solid var(--theme-border)", fontSize: 13 }} />
        </label>
        <label style={{ fontSize: 12, display: "grid", gap: 4 }}>
          Aggregate type
          <input value={aggregateType} onChange={(e) => setAggregateType(e.target.value)} placeholder="Treatment" style={{ padding: "8px 10px", borderRadius: "var(--theme-radius-card)", border: "1px solid var(--theme-border)", fontSize: 13 }} />
        </label>
        <label style={{ fontSize: 12, display: "grid", gap: 4 }}>
          Aggregate id
          <input value={aggregateId} onChange={(e) => setAggregateId(e.target.value)} placeholder="business id" style={{ padding: "8px 10px", borderRadius: "var(--theme-radius-card)", border: "1px solid var(--theme-border)", fontSize: 13 }} />
        </label>
      </div>
      ...
      {error && <div role="alert" style={{ fontSize: 13, color: "var(--theme-danger)", background: "color-mix(in srgb,var(--theme-danger) 10%,transparent)", padding: 10, borderRadius: "var(--theme-radius-card)", marginBottom: 12 }}>⚠️ {error}</div>}
              ...
              {entries.map((entry) => {
                ...
                return (
                  <tr key={`${entry.traceKind}:${entry.traceId}`}>
                    ...
                    <td><span style={{ padding: "2px 8px", borderRadius: "var(--theme-radius-card)", fontSize: 11, fontWeight: 800, color: tone.fg, background: tone.bg }}>{entry.traceKind}</span></td>
                    <td>{entry.state ?? "—"}</td>
      ...
      {selected && (
        <div role="presentation" onClick={() => setSelected(null)} style={{ position: "fixed", inset: 0, zIndex: 120, background: "rgba(15,23,42,.6)", backdropFilter: "blur(6px)", display: "grid", placeItems: "center", padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: "min(860px, 100%)", maxHeight: "90vh", overflowY: "auto", background: "var(--theme-surface-raised)", border: "1px solid var(--theme-border)", borderRadius: "var(--theme-radius-card)", padding: 28 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", marginBottom: 16 }}>
              ...
              <button type="button" onClick={() => setSelected(null)} aria-label="Close" style={{ border: "1px solid var(--theme-border)", background: "transparent", borderRadius: "var(--theme-radius-card)", width: 32, height: 32, cursor: "pointer", fontSize: 16 }}>✕</button>
            </div>
            ...
            <div style={{ border: "1px solid var(--theme-border)", borderRadius: "var(--theme-radius-card)", padding: 14, marginBottom: 16 }}>
            ...
            <div>
              ...
              <pre style={{ overflowX: "auto", background: "var(--theme-surface-muted)", color: "var(--theme-text-primary)", padding: 14, borderRadius: "var(--theme-radius-card)", fontSize: 12, lineHeight: 1.5 }}>{JSON.stringify(selected.metadata, null, 2)}</pre>
            </div>
          </div>
        </div>
      )}
