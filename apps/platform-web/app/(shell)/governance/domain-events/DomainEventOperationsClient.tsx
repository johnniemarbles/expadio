...
import motionStyles from './DomainEventOperationsClient.module.css';

type StatusFilter = 'ALL' | DomainEventOperationItem['status'];

interface Payload {
  items: DomainEventOperationItem[];
  counts: { total: number; dead: number; failed: number; claimed: number; pending: number; published: number };
}

const FILTERS: readonly StatusFilter[] = ['ALL', 'DEAD', 'FAILED', 'CLAIMED', 'PENDING', 'PUBLISHED'];

function fmt(value: string | null): string { return value ? new Date(value).toLocaleString() : '—'; }
...
export function DomainEventOperationsClient({ initial }: { initial: Payload }) {
  ...
  const items = useMemo(() => filter === 'ALL' ? payload.items : payload.items.filter((item) => item.status === filter), [filter, payload.items]);

  async function reload() {
    const response = await fetch('/api/governance/domain-events?limit=200', { cache: 'no-store' });
  ...
  async function requeue(item: DomainEventOperationItem) {
    const reason = prompt('Why should this terminal Domain Event be requeued? This reason is written to the immutable audit record.')?.trim();
    if (!reason) return;
    if (!confirm(`Requeue ${item.eventType}? This starts a new delivery retry cycle.`)) return;

    setBusy(item.outboxId); setNotice(null); setError(null);
    try {
      const response = await fetch(`/api/governance/domain-events/${encodeURIComponent(item.outboxId)}/requeue`, {
        method: 'POST',
        headers: { 'content-type': 'application/json', 'x-expadio-reauth-at': new Date().toISOString() },
        body: JSON.stringify({ reason }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error ?? data.message ?? 'Requeue failed.');
  ...
  return (
    <main className={motionStyles.surface} style={{ display: 'grid', gap: 18 }}>
      <section>
        <p style={{ margin: 0, fontSize: 11, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.08em', color: 'var(--theme-primary)' }}>Governance · event operations</p>
        <h1 style={{ margin: '5px 0 6px' }}>Domain Event delivery</h1>
        <p style={{ margin: 0, color: 'var(--theme-text-secondary)', maxWidth: 850 }}>Inspect tenant-scoped event delivery, retries and terminal dead letters. Domain Events are immutable; requeue only starts a new outbox delivery cycle and records who authorized it.</p>
      </section>

      <section className={motionStyles.stats} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10 }}>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.outboxId} style={{ borderTop: '1px solid var(--theme-surface-muted)' }}>
                ...
                <td style={{ padding: 12 }}><div style={{ fontWeight: 700 }}>{item.eventType}</div><div style={{ fontSize: 11, color: 'var(--theme-text-muted)', fontFamily: 'monospace' }}>{item.eventId}</div></td>
                <td style={{ padding: 12 }}><div>{item.aggregateType}</div><div style={{ fontSize: 11, color: 'var(--theme-text-muted)', fontFamily: 'monospace' }}>{item.aggregateId}</div></td>
                <td style={{ padding: 12, fontWeight: 700 }}>{item.attempts}</td>
                <td style={{ padding: 12, fontSize: 12 }}>{fmt(item.availableAt)}</td>
                <td style={{ padding: 12, maxWidth: 320, color: item.lastError ? 'var(--theme-danger)' : 'var(--theme-text-muted)', fontSize: 12 }}>{item.lastError ?? '—'}</td>
                <td style={{ padding: 12 }}>
            ...
            {items.length === 0 ? <tr><td colSpan={7} style={{ padding: 24, textAlign: 'center', color: 'var(--theme-text-secondary)' }}>No Domain Event outbox items match this filter.</td></tr> : null}
          </tbody>
        </table>
      </section>
    </main>
