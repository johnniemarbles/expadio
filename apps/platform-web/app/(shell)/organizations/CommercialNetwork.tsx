  ...
  return <div className={styles.stack}>
    ...
    <section className={styles.section}>
      ...
      <div className={styles.tableWrap}><table><thead><tr><th>Agreement</th><th>Parties</th><th>Kind</th><th>State</th><th>Action</th></tr></thead><tbody>
        {data.agreements.map((agreement)=><tr key={agreement.enterprise_commercial_agreement_id}>
          ...
          <td>{agreement.state!=='ACTIVE'?<button disabled={busy!==null} onClick={()=>{const evidence=window.prompt('Enter the executed-agreement evidence reference.');if(!evidence?.trim())return;void act(`agreement:${agreement.enterprise_commercial_agreement_id}`,{action:'ACTIVATE_AGREEMENT',agreementId:agreement.enterprise_commercial_agreement_id,executionEvidenceRefs:[evidence.trim()]},'Agreement activated.');}}>Activate with evidence</button>:<span className={styles.good}>Active</span>}</td>
        </tr>)}
      </tbody></table></div>
    </section>
    ...
    <section className={styles.section}>
      ...
      <div className={styles.cards}>{data.appointments.map((appointment)=><article key={appointment.enterprise_appointment_id}>
        ...
        <footer><b>{appointment.state}</b>
          ...
          {appointment.state==='APPROVED'&&<button disabled={busy!==null} onClick={()=>{const evidence=window.prompt('Enter the rights-issuance evidence reference.');if(!evidence?.trim())return;void act(`rights:${appointment.enterprise_appointment_id}`,{action:'ISSUE_APPOINTMENT_RIGHTS',appointmentId:appointment.enterprise_appointment_id,evidenceRefs:[evidence.trim()]},'Rights issued and appointment activated.');}}>Issue rights</button>}
          {appointment.state==='ACTIVE'&&appointment.territories.map((territory)=>{const existing=data.jurisdictions.find((j)=>j.enterprise_appointment_id===appointment.enterprise_appointment_id&&j.territory_id===territory.territoryId);return !existing?<button key={territory.territoryId} disabled={busy!==null} onClick={()=>{const evidence=window.prompt('Enter the jurisdiction activation request evidence reference.');if(!evidence?.trim())return;void act(`jurisdiction:${appointment.enterprise_appointment_id}:${territory.territoryId}`,{action:'START_JURISDICTION_ACTIVATION',appointmentId:appointment.enterprise_appointment_id,territoryId:territory.territoryId,evidenceRefs:[evidence.trim()]},'Jurisdiction activation review started.');}}>Activate {territory.name}</button>:null;})}
        </footer>
      </article>)}</div>
    </section>
    ...
    <section className={styles.section}>
      ...
      <div className={styles.cards}>{data.jurisdictions.map((jurisdiction)=><article key={jurisdiction.enterprise_jurisdiction_activation_id}>
        ...
        <footer><b>{jurisdiction.state}</b>{jurisdiction.state!=='ACTIVE'&&<button disabled={busy!==null} onClick={()=>{const evidence=window.prompt('Enter the evidence dossier reference covering agreement, rights, access, compliance, and operational readiness.');if(!evidence?.trim())return;if(!window.confirm('Confirm all five activation controls are satisfied by this evidence dossier.'))return;const ref=evidence.trim();const assessments=['AGREEMENT','RIGHTS','ACCESS','COMPLIANCE','OPERATIONAL_READINESS'].map((dimension)=>({dimension,outcome:'SATISFIED',reason:`Operator confirmed ${dimension.toLowerCase().replaceAll('_',' ')} against the supplied evidence dossier.`,evidenceRefs:[ref]}));void act(`verify:${jurisdiction.enterprise_jurisdiction_activation_id}`,{action:'VERIFY_AND_ACTIVATE_JURISDICTION',jurisdictionActivationId:jurisdiction.enterprise_jurisdiction_activation_id,reason:'All jurisdiction activation controls were explicitly verified against the supplied evidence dossier.',assessments,evidenceRefs:[ref]},'Jurisdiction activated.');}}>Verify controls & activate</button>}</footer>
