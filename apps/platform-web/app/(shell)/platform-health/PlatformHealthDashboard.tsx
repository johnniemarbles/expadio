...
function gaugeColor(score:number):string { return score===0?"#71717a":score<30?"#fb7185":score<60?"#f59e0b":score<90?"#8b5cf6":"#facc15"; }
  ...
  return <div className={styles.commandCenter}>
    ...
    <main className={styles.workspace}>{error?<div className={styles.errorBanner} role="alert">Telemetry link error: {error}</div>:null}
      ...
      <div className={styles.bentoGrid}>
        <SpotlightCard className={styles.topologyCard} stagger={4}><div className={styles.cardHeader}><div><h2>Governed Execution Topology</h2><p>Live read model · no worker mutations</p></div><span className={styles.microBadge}>4 nodes linked</span></div><div className={styles.topologyCanvas}><div className={[styles.hudCorner,styles.hudCornerTop].join(" ")}>MAP 600×456<br/>ORIGIN 300,228</div><div className={[styles.hudCorner,styles.hudCornerBottom].join(" ")}>REFRESH {loadedAt?formatLogTime(loadedAt):"PENDING"}<br/>SCOPE TENANT-BOUND</div><svg className={styles.topologySvg} viewBox="0 0 600 456" role="img" aria-labelledby="topology-title topology-desc"><title id="topology-title">EXPADIO governed execution topology</title><desc id="topology-desc">Four health domains connected to the governed action fabric.</desc><defs><radialGradient id="sweepGradient"><stop offset="0" stopColor="#facc15" stopOpacity=".2"/><stop offset=".6" stopColor="#facc15" stopOpacity=".035"/><stop offset="1" stopColor="#facc15" stopOpacity="0"/></radialGradient></defs><circle className={styles.sweep} cx="300" cy="228" r="190"/><circle className={styles.orbit} cx="300" cy="228" r="154"/><circle className={[styles.orbit,styles.orbitSecondary].join(" ")} cx="300" cy="228" r="196"/>{DOMAINS.map((domain)=><line key={`line-${domain.key}`} className={styles.connection} x1="300" y1="228" x2={domain.x} y2={domain.y}/>)}<polygon className={styles.centerHex} points="300,180 342,204 342,252 300,276 258,252 258,204"/><text className={styles.centerText} x="300" y="224">GOVERNED</text><text className={styles.centerText} x="300" y="238">ACTION FABRIC</text>{DOMAINS.map((domain)=>{const loaded=domains.find((item)=>item.key===domain.key),active=loaded?.entries.filter((entry)=>entry.itemCount>0)??[],status=loaded?.error?"CRITICAL":worstStatus(active);return <g key={domain.key}><circle className={nodeRingClass(status)} cx={domain.x} cy={domain.y} r="29"/><circle className={nodeCoreClass(status)} cx={domain.x} cy={domain.y} r="4"/><text className={styles.nodeLabel} x={domain.x} y={domain.y+47}>{domain.label.toUpperCase()}</text><text className={styles.nodeCount} x={domain.x} y={domain.y+60}>{loaded?.error??`${totalItems(active)} OPEN`}</text></g>;})}</svg></div></SpotlightCard>
