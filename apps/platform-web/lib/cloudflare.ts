/**
 * Minimal Cloudflare DNS client for sending-domain auto-configuration.
 *
 * Given an API token (from the operator or the deployment env) it discovers the
 * zone that owns a domain, then idempotently creates or updates each record —
 * so re-running auto-configure converges instead of duplicating. The token is
 * used transiently for the request and never persisted or logged here.
 */
...
async function cf(token: string, path: string, init?: RequestInit): Promise<any> {
  ...
  if (!res.ok || body?.success === false) {
    ...
    // 401/403 → the token is the problem; surface it as a client error.
...
/** Candidate zone names for a domain, most specific first (no PSL, best-effort). */
