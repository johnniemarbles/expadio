...
-- Runtime repair for installations whose historical schema_migrations table
-- was backfilled too broadly. This migration is intentionally idempotent and
-- reconstructs the Domain Event + Transactional Outbox spine when absent.
...
CREATE INDEX IF NOT EXISTS domain_event_outbox_tenant_idx
...
DROP TRIGGER IF EXISTS domain_events_append_only ON platform.domain_events;
...
DROP POLICY IF EXISTS domain_events_tenant_select ON platform.domain_events;
CREATE POLICY domain_events_tenant_select
  ON platform.domain_events
  FOR SELECT
  USING (tenant_id = platform.current_tenant_id());

DROP POLICY IF EXISTS domain_events_tenant_insert ON platform.domain_events;
CREATE POLICY domain_events_tenant_insert
  ON platform.domain_events
  FOR INSERT
  WITH CHECK (tenant_id = platform.current_tenant_id());

DROP POLICY IF EXISTS domain_event_outbox_tenant_all ON platform.domain_event_outbox;
