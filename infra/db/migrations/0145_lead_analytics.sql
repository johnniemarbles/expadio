...
JOIN platform.organization_closure oc
  ON oc.descendant_id = lcl.organization_id
 AND oc.tenant_id     = lcl.tenant_id
GROUP BY oc.ancestor_id, lcl.tenant_id;
...
JOIN platform.organization_closure oc
  ON oc.descendant_id = lt.organization_id
 AND oc.tenant_id     = lt.tenant_id
GROUP BY oc.ancestor_id, lt.tenant_id, lt.priority, lt.status;
...
JOIN platform.organization_closure oc
  ON oc.descendant_id = la.organization_id
 AND oc.tenant_id     = la.tenant_id
GROUP BY oc.ancestor_id, la.tenant_id, la.activity_type, date_trunc('day', la.occurred_at);
...
JOIN platform.organization_closure oc
  ON oc.descendant_id = lcl.organization_id
 AND oc.tenant_id     = lcl.tenant_id
GROUP BY oc.ancestor_id, lcl.tenant_id, lcs.channel, lcs.surface;
